...
import idgActor.Controllers.bufferedSocket as bufferedSocket
...
class SocketIO(DeviceIO):
    ...
    def _connect(self, cmd=None, timeout=1.0):
        ...
        try:
            ...
            if cmd is not None:
                cmd.warn(f'text="failed to connect socket ({self.host}:{self.port}):{e}"')
            raise
...
class temps(object):
    def __init__(self, actor, name,
        ...
        host = self.actor.config.get('thermalTemps', 'host')
        port = int(self.actor.config.get('thermalTemps', 'port'))
